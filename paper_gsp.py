...
class SRSO:
    ...
    def __init__(self, name):
        ...
        self.C_k_min, self.C_k_max = 4000, 6000         # 计算能力 = 5000
        self.P_k_min, self.P_k_max = 150, 200           # 计算功率 = 200
        self.h_k_min, self.h_k_max = 0.00025, 0.00075   # 损失函数 = 0.00005
        self.d_k_min, self.d_k_max = 1500, 3500         # 计算距离 = 2000
    ...
    def SRSO_optimal_test(self):
        for average_time in range(1, 5000, 1):
            self.C_k_list = SRSO.ra_sample(self, self.C_k_min, self.C_k_max, self.num_relay_max)
            self.P_k_list = SRSO.ra_sample(self, self.P_k_min, self.P_k_max, self.num_relay_max)
            self.h_k_list = SRSO.ra_sample_float(self, self.h_k_min, self.h_k_max, self.num_relay_max)
            self.d_k_list = SRSO.ra_sample(self, self.d_k_min, self.d_k_max, self.num_relay_max)
    ...
    def SRSO_RS_test(self):
        for average_time in range(1, 5000, 1):
            ...
            self.i_ratio_list = []
            self.i_ratio_list_d = []
            self.result_sequence_rs = []
            self.x_01, self.x_1i = 0, 0

            SRSO.available_prob_list(self)  # 服务器是否可用de概率
            SRSO.available_pro_list_d(self)  # 服务器是否可用de概率
            for num_relay in self.num_relay_list:
                server_list = [i for i in range(0, num_relay)]
                E_detect = self.tau * self.P_d
                E_1_tran, E_1_cal, E_2_tran, E_2_cal = 0, 0, 0, 0
                num_rs_detect = 1
                while 1:
                    ...
                    if flag_x1 == 0:
                        ...
                        self.x_01 = 1
                        break                                   # 若都不可用，则跳出循环，只能在本地计算
                    num_random = random.choice(server_list)     # 随机选择的第num_random + 1 个服务器在列表中的位置
                    i = num_random
                    if self.i_ratio_list[i] <= self.enable_ratio:  # 可用，确定计算量x_1i
                        self.x_1i = (self.t_limit * self.C_k_list[i]) / (self.V * self.P_k_list[i])
                        if self.x_1i > 1:                       # 如果完全卸载到relay，没有超过限制，那就完全卸载到该处
                            self.x_1i = 1
                            # print('total_offloading')
                        E_1_tran, E_1_cal = SRSO.relay_energy_1(self, self.x_01, self.x_1i, self.h_k_list[i],
                                                                self.P_k_list[i], self.C_k_list[i], self.d_k_list[i])
                        # print(E_1_tran, E_1_cal)
                        break
                    num_rs_detect = num_rs_detect + 1

                # self.x_01 = 1，无relay可用，只能本地卸载；x_1i有值但不等于1，部分relay卸载，进行二层检测；else，x_1i等于1，则完全relay卸载
                if self.x_01 == 1:                                         # 无relay可用，只能本地卸载
                    E_local = self.x_01 * self.P_l * self.V / self.C_l
                    E_final = E_local + E_detect * num_rs_detect
                elif (self.x_01 + self.x_1i) != 1:                           # 若relay没卸载完，则继续第二层随机
                    len_remote = random.choice(self.num_remote_list)         # 随机选择对应远端集合里的remote数量
                    server_d_list = [j for j in range(0, len_remote)]          # 生成列表，在里面随机选择一个数
                    while 1:
                        flag_x2 = 0
                        for x in range(0, len_remote):
                            if self.i_ratio_list_d[x] <= self.enable_ratio:
                                ...
                                flag_x2 = flag_x2
                        if flag_x2 == 0:                               # relay没卸载完，remote没有可用的，则只能将剩下的传回本地卸载
                            # print('no_server_can_use')
                            E_2_tran = 0
                            E_2_cal = 0
                            self.x_01 = 1 - self.x_1i                # 剩下的任务还是只能本地计算
                            break                                    # 若都不可用，则跳出循环，只能在relay计算
                        num_random_d = random.choice(server_d_list)  # 随机选择的第num_random + 1 个服务器在列表中的位置
                        j = num_random_d
                        if self.i_ratio_list_d[j] <= self.enable_ratio:
                            E_2_tran, E_2_cal = SRSO.remote_energy_1(self, self.x_01, self.x_1i,
                                                                     self.h_d_k_list[j], self.P_d_k_list[j],
                                                                     self.C_d_k_list[j], self.d_d_k_list[j])
                            # print(E_2_tran, E_2_cal)
                            break
                        num_rs_detect = num_rs_detect + 1
                    if self.x_01 != 0:                               # 说明剩下的任务还是只能本地计算
                        E_local = self.x_01 * self.V * self.P_l / self.C_l
                        E_final = E_1_tran + E_1_cal + E_local + E_detect * num_rs_detect
                    else:
                        E_final = E_1_tran + E_1_cal + E_2_tran + E_2_cal + E_detect * num_rs_detect
                else:
                    E_final = E_1_tran + E_1_cal + E_detect + num_rs_detect
                self.result_sequence_rs.append(E_final)
                # 为了计算optimal循环后的均方值
            self.result_matrix_rs.append(self.result_sequence_rs)
        return self.result_matrix_rs
    ...
    def SRSO_GSP_test(self):
        for average_time in range(1, 5000, 1):
            ...
            for num_relay in self.num_relay_list:
                server_list = [i for i in range(0, num_relay)]
                temp_d_list = []
                for xx in server_list:
                    temp_d_list.append(self.d_k_list[xx])
                E_detect = self.tau * self.P_d
                E_1_tran, E_1_cal, E_2_tran, E_2_cal = 0, 0, 0, 0
                # num_rs_detect = 1
                while 1:
                    ...
                    if flag_x1 == 0:
                        ...
                        break                                   # 若都不可用，则跳出循环，只能在本地计算
                    # num_random = random.choice(server_list)     # 随机选择的第num_random + 1 个服务器在列表中的位置
                    num_min_distance = np.argmin(temp_d_list)
                    i = num_min_distance
                    if self.i_ratio_list[i] <= self.enable_ratio:  # 可用，确定计算量x_1i
                        self.x_1i = (self.t_limit * self.C_k_list[i]) / (self.V * self.P_k_list[i])
                        if self.x_1i > 1:                       # 如果完全卸载到relay，没有超过限制，那就完全卸载到该处
                ...
                if self.x_01 == 1:                                         # 无relay可用，只能本地卸载
                    ...
                    server_d_list = [j for j in range(0, len_remote)]          # 生成列表，在里面随机选择一个数
                    temp_d_d_list = []
                    for xxx in server_d_list:
                        temp_d_d_list.append(self.d_d_k_list[xxx])
                    while 1:
                        flag_x2 = 0
                        for x in range(0, len_remote):
                            if self.i_ratio_list_d[x] <= self.enable_ratio:
                        ...
                        if flag_x2 == 0:                               # relay没卸载完，remote没有可用的，则只能将剩下的传回本地卸载
                            ...
                            break                                    # 若都不可用，则跳出循环，只能在relay计算
                        # num_random_d = random.choice(server_d_list)  # 随机选择的第num_random + 1 个服务器在列表中的位置
                        num_min_distance_d = np.argmin(temp_d_d_list)
                        j = num_min_distance_d
                        if self.i_ratio_list_d[j] <= self.enable_ratio:
                            E_2_tran, E_2_cal = SRSO.remote_energy_1(self, self.x_01, self.x_1i,
                                                                     self.h_d_k_list[j], self.P_d_k_list[j],
                                                                     self.C_d_k_list[j], self.d_d_k_list[j])
                            ...
                            temp_d_d_list[j] = 10000
                        # num_rs_detect = num_rs_detect + 1
                    if self.x_01 != 0:                               # 说明剩下的任务还是只能本地计算
                        E_local = self.x_01 * self.V * self.P_l / self.C_l
                        E_final = E_1_tran + E_1_cal + E_local + E_detect * num_relay
                    else:
                        E_final = E_1_tran + E_1_cal + E_2_tran + E_2_cal + E_detect * (num_relay + len_remote)
                else:
                    E_final = E_1_tran + E_1_cal + E_detect * num_relay
                self.result_sequence_gsp.append(E_final)
                # 为了计算optimal循环后的均方值
            self.result_matrix_gsp.append(self.result_sequence_gsp)
        return self.result_matrix_gsp
...
plt.plot(d_time_rs, e_consu_rs, color='pink', alpha=0.8,
         marker=line_mark[15], linestyle='-',
         label="RRRS", markersize=10, linewidth=3, clip_on=False)
plt.plot(d_time_gsp, e_consu_gsp, color='blue', alpha=0.3,
         marker=line_mark[15], linestyle='-',
         label="SDSS", markersize=10, linewidth=3, clip_on=False)
plt.plot(d_time_optimal, e_consu_optimal, color='red', alpha=0.5,
         marker=line_mark[12], linestyle='-',
         label="Optimal SRSO", markersize=10, linewidth=3, clip_on=False)


plt.xlim(2, 20)
plt.legend(fontsize=10)
plt.grid(linestyle='-.')     # 添加网格
plt.savefig("for_review_paper.png", dpi=500, bbox_inches='tight')    # 解决图片不清晰，不完整的问题
plt.show()

# width = 0.5                    # 设置柱与柱之间的宽度
# fig, ax = plt.subplots()
# ax.bar(d_time_rs, e_consu_rs, width, alpha=0.5, color='green', label="RRRS")
# ax.bar(d_time_gsp + width, e_consu_gsp, width, alpha=0.7, color='red', label="SDSS")
# ax.bar(d_time_optimal + width + width, e_consu_optimal, width, alpha=0.7, color='blue', label="Optimal SRSO")
# ax.set_xticks(d_time_gsp + width)      # 将坐标设置在指定位置
# ax.set_xticklabels(d_time_optimal)           # 将横坐标替换成
# plt.ylabel('Average energy consumption')  # 纵坐标轴标题
# plt.xlabel("Number of relay servers")
# # plt.ylim(380, 520)              # 3,6,9,12,15,18
# plt.ylim(580, 660)              # 情况二
# plt.legend(fontsize=9)
# plt.grid(linestyle='-.')     # 添加网格
# # plt.savefig("Figure_result_1.png", dpi=500, bbox_inches='tight')    # 解决图片不清晰，不完整的问题
# plt.show()
