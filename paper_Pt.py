...
class SRSO:
    ...
    def __init__(self, name):
        ...
        self.P_t, self.P_t2 = 50, 50  # 传输功率为确定值 mW
        self.P_t_2, self.P_t2_2 = 60, 60
        self.P_t_3, self.P_t2_3 = 70, 70
        self.P_d = 1.0  # 信道检测功率
        self.tau = 0.05
    ...
    def relay_energy_1(self, x_01, x_1i, h_k, P_k, C_k, d_k, P_t):
        v_1i = self.w * np.log2(1 + P_t * (np.abs(h_k)) ** 2 / ((1 + d_k ** self.alpha_1) * self.sigma_1 ** 2))
        # print(v_1i)
        E_relay_tran = (1 - x_01) * (P_t * self.D / v_1i)
        E_relay_cal = x_1i * (self.V * P_k / C_k)
        return E_relay_tran, E_relay_cal
    # 计算上传到Remote端所消耗的能量（返回值为：传输能耗 and 计算能耗）    1000~125
    def remote_energy_1(self, x_01, x_1i, h_d_k, P_d_k, C_d_k, d_d_k, P_t2):
        v_ij = self.w * np.log2(1 + P_t2 * (np.abs(h_d_k)) ** 2 / ((1 + d_d_k ** self.alpha_1) * self.sigma_1 ** 2))
        # print(v_ij)
        E_remote_tran = (1 - x_01 - x_1i) * (P_t2 * self.D / v_ij)
                        ...
                        if i == 0:
                            ...
                            if self.i_ratio_list[i] <= self.enable_ratio:
                                ...
                                if self.x_1i > 1:  # 如果完全卸载到relay，没有超过时间限制，则就完全卸载到该处。
                                    self.x_1i = 1
                                E_1_tran, E_1_cal = SRSO.relay_energy_1(self, self.x_01, self.x_1i, self.h_k_list[i], self.P_k_list[i], self.C_k_list[i], self.d_k_list[i], self.P_t)
                            ...
                            if j == 0:
                                ...
                                if self.i_ratio_list_d[j] <= self.enable_ratio:
                                    ...
                                    E_2_tran, E_2_cal = SRSO.remote_energy_1(self, self.x_01, self.x_1i,
                                                                            ...
                                                                            self.C_d_k_list[j], self.d_d_k_list[j], self.P_t2)
                    ...
                    for i in range(0, num_relay):
                        if i == 0:
                            ...
                            if self.i_ratio_list[i] <= self.enable_ratio:
                                ...
                                if self.x_1i > 1:  # 如果完全卸载到relay，没有超过时间限制，则就完全卸载到该处。
                                    self.x_1i = 1
                                E_1_tran, E_1_cal = SRSO.relay_energy_1(self, self.x_01, self.x_1i, self.h_k_list[i], self.P_k_list[i], self.C_k_list[i], self.d_k_list[i], self.P_t_2)
                                E_final_1 = E_detect + E_1_tran + E_1_cal
                                E_r.append(E_final_1)
                                #print('1', self.x_01, self.x_1i)
                            else:
                        ...
                        for j in range(0, len_remote):
                            if j == 0:
                                ...
                                if self.i_ratio_list_d[j] <= self.enable_ratio:
                                    ...
                                    E_2_tran, E_2_cal = SRSO.remote_energy_1(self, self.x_01, self.x_1i,
                                                                            self.h_d_k_list[j], self.P_d_k_list[j],
                                                                            self.C_d_k_list[j], self.d_d_k_list[j], self.P_t_2)
                                    E_final_2 = E_detect + E_2_tran + E_2_cal
                                    E_d.append(E_final_2)
                                    # print('1', self.x_01, self.x_1i)
                                else:
                    ...
                    for i in range(0, num_relay):
                        if i == 0:
                            ...
                            if self.i_ratio_list[i] <= self.enable_ratio:
                                ...
                                if self.x_1i > 1:  # 如果完全卸载到relay，没有超过时间限制，则就完全卸载到该处。
                                    self.x_1i = 1
                                E_1_tran, E_1_cal = SRSO.relay_energy_1(self, self.x_01, self.x_1i, self.h_k_list[i], self.P_k_list[i], self.C_k_list[i], self.d_k_list[i], self.P_t_3)
                                E_final_1 = E_detect + E_1_tran + E_1_cal
                                E_r.append(E_final_1)
                                #print('1', self.x_01, self.x_1i)
                            else:
                        ...
                        for j in range(0, len_remote):
                            if j == 0:
                                ...
                                if self.i_ratio_list_d[j] <= self.enable_ratio:
                                    ...
                                    E_2_tran, E_2_cal = SRSO.remote_energy_1(self, self.x_01, self.x_1i,
                                                                            self.h_d_k_list[j], self.P_d_k_list[j],
                                                                            self.C_d_k_list[j], self.d_d_k_list[j], self.P_t2_3)
                                    E_final_2 = E_detect + E_2_tran + E_2_cal
                                    E_d.append(E_final_2)
                                    # print('1', self.x_01, self.x_1i)
                                else:
...
plt.plot(d_time_optimal, e_consu_optimal, color='blue', alpha=0.7,
         ...
         label=r'$P_{tr}$=50mW', markersize=13, linewidth=2, clip_on=False)

plt.plot(d_time_rs, e_consu_rs, color='red', alpha=0.6,
         marker=line_mark[3], linestyle='-',
         label=r'$P_{tr}$=60mW', markersize=8, linewidth=2, clip_on=False)
plt.plot(d_time_gsp, e_consu_gsp, color='green', alpha=0.5,
         marker=line_mark[12], linestyle='-',
         label=r'$P_{tr}$=70mW', markersize=8, linewidth=2, clip_on=False)
plt.xlim(3, 18)
# plt.ylim(430, 660)
plt.legend(fontsize=10)
plt.grid(linestyle='-.')     # 添加网格
# plt.savefig("Figure_result_4.png", dpi=500, bbox_inches='tight')    # 解决图片不清晰，不完整的问题
plt.show()
